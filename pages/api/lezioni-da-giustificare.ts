  ...
  async ({ req }) => {
    const lezioni = await prisma.lezione.findMany({
      where: {
        ...
        libretto: {
          in: [Libretto.LEZIONE_SALTATA, Libretto.ASSENTE_GIUSTIFICATO],
        },
...
const recuperaLezione = endpoint(
  ...
  async ({ body }) => {
    ...
    return await prisma.$transaction(async (tx) => {
      const lezioneDaRecuperare = await tx.lezione.findUnique({
        where: { id: idDaRecuperare },
        include: { alunni: true },
      });
      if (!lezioneDaRecuperare) return { status: 404 };
      ...
      const lezione = {
        ...
        orarioDiFine: (() => {
          ...
          const timeDiff =
            lezioneDaRecuperare.orarioDiFine.getTime() -
            lezioneDaRecuperare.orarioDiInizio.getTime();
      ...
      await tx.lezione.create({
        data: {
          ...
          alunni: {
            connect: lezione.alunniIds.map((id) => {
              return { id };
            }),
          },
          orarioDiInizio: lezione.orarioDiInizio,
...
export default withIronSessionApiRoute(
  asHandler([getLezioniDaRecuperare, recuperaLezione]),
  sessionOptions
);
