...
const addLezioni = endpoint(
  ...
  async ({ body }) => {
    const lezioni = body.map((lezione) => {
      return {
        ...lezione,
        orarioDiInizio: lezione.orario,
        orarioDiFine: (() => {
          const orarioDiFine = new Date(lezione.orario);
          orarioDiFine.setMinutes(
            orarioDiFine.getMinutes() + Number(lezione.durataInMin)
          );
          return orarioDiFine;
        })(),
      };
    });
    const alunniToLezioni = Array.from(
      lezioni
        .reduce(
          (map, lezione) => {
            lezione.alunniIds.forEach((alunnoId) => {
              if (!map.get(alunnoId)) map.set(alunnoId, []);
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      await Promise.all(
        alunniToLezioni.map(([alunnoId, lezioniIn]) =>
          tx.alunno.update({
            data: {
              lezioni: {
...
export default withIronSessionApiRoute(
  asHandler([addLezioni, deleteLezioni]),
  sessionOptions
);
