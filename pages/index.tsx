...
export const getServerSideProps = requireAuth<Props>(
...
const Home: NextPage<Props> = () => {
  ...
  const { data: lezioniDaRecuperare, mutate: mutateLezioniDaGiustificare } =
    useSWR<Lezione[]>("/api/lezioni-da-giustificare", async (url: string) => {
  ...
  const { data: finireLaCompilazione, mutate: mutateFinireLaCompilazione } =
    useSWR<Lezione[]>("/api/lezioni-da-compilare", async (url: string) => {
      const { parser } = await zodFetch(url, {
        method: "GET",
        headers: { "Content-Type": "application/json" },
      ...
      <Layout requiresAuth loading={!lezioni}>
        <Container fluid className={styles.container}>
          <main className={styles.main}>
            ...
            {finireLaCompilazione && (finireLaCompilazione?.length ?? 0) > 0 && (
              <Row className="align-items-center">
                <div className="alert alert-warning" role="alert">
                  ...
                  {finireLaCompilazione[
                    finireLaCompilazione.length - 1
                  ].orarioDiInizio.toLocaleDateString(undefined, {
                    year: "numeric",
                    month: "2-digit",
                    day: "2-digit",
                  })}
            ...
            <LezioniTable
              scrollable
              content={useMemo(() => lezioni ?? [], [lezioni])}
        ...
        handleSubmit={async (lezioneGiustificata) => {
          const { res, parser } = await zodFetch(
            "/api/lezioni-da-giustificare",
            {
              ...
              responseValidator:
                LezioniDaGiustificareApi.Post.ResponseValidator,
            }
          );
