...
type Props = {
    docenti: (Docente & { hoursDone: number } & { euros: number } & { eurosPaid: number })[];
};
        ...
        props: {
            docenti: (await prisma.docente.findMany({})).map((docente) => {
                return {
                    ...
                    euros: ((docenteIdToMinutes.get(docente.id) ?? 0) / 60) * docente.stipendioOrario, // TODO: actually use the hourly rate
                    eurosPaid: ((paidLessons.get(docente.id) ?? 0) / 60) * docente.stipendioOrario, // TODO: actually use the hourly rate
                };
            }),
        },
    };
