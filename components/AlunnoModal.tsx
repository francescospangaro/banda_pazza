    ...
    return (
        <Modal show={show} onHide={handleClose}>
            <Modal.Header closeButton>
                <Modal.Title>{(isEdit ? 'Modifica' : 'Aggiungi') + ' alunno'}</Modal.Title>
            </Modal.Header>
            ...
            <Form onSubmit={async (e) => {
                ...
                if(isEdit) {
                    ...
                    if(newAlunno.cognome === alunno?.cognome) newAlunno.cognome = undefined;
                    if(newAlunno.email === alunno?.email) newAlunno.email = undefined;
                    if(newAlunno.cf === alunno?.cf) newAlunno.cf = undefined;
                    if(newAlunno.docenteId === alunno?.docenteId) newAlunno.docenteId = undefined;
                }
                ...
                <Modal.Body>
                    <div className="row g-3">
                        <Col xs="12">
                            ...
                            <Form.Control type="text" required name="nome" defaultValue={alunno?.nome} />
                        </Col>
                        <Col xs="12">
                            <Form.Label>Cognome</Form.Label>
                            <Form.Control type="text" required name="cognome" defaultValue={alunno?.cognome} />
                        </Col>
                        <Col xs="12">
                            <Form.Label>Codice Fiscale</Form.Label>
                            <Form.Control type="text" required name="cf" defaultValue={alunno?.cf} />
                        </Col>
                        <Col xs="12">
                            <Form.Label>Email</Form.Label>
                            <Form.Control type="email" required name="email" defaultValue={alunno?.email} />
                        </Col>
                        <Col xs="12">
                            <Form.Label>Docente</Form.Label>
                            <Form.Select required name="docente" defaultValue={alunno?.docenteId} >
                                <>
                                    <option></option>
                                    {docenti.map(docente => {
                                        return <option key={docente.id} value={docente.id}>
                ...
                <Modal.Footer>
                    ...
                    <Button type="submit" disabled={executing} variant="primary">{isEdit ? 'Modifica' : 'Aggiungi'}</Button>
                </Modal.Footer>
            </Form>
        </Modal>
    );
