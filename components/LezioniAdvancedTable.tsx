...
type Props = {
  ...
  onEditLezione?: (editedLezioniField: {
    id: number;
...
export default function LezioniAdvancedTable({
  ...
  const tableData: TableLezione[] = useMemo(
    () =>
      content.map((lezione) => {
        return {
          in: lezione,
          ...
          note:
            ...
            (lezione.recuperoDi
              ? "Recupero del " +
                lezione.recuperoDi?.orarioDiInizio.toLocaleDateString(
                  undefined,
            ...
            (lezione.recuperataDa
              ? "Recuperata il " +
                lezione.recuperataDa?.orarioDiInizio.toLocaleDateString(
                  undefined,
                  { year: "numeric", month: "2-digit", day: "2-digit" }
                )
              : ""),
    ...
    () => [
        ...
        Cell: (props) => {
          return (
            <Form.Check
              ...
              onChange={(e) => {
                if (onSelectLezione)
                  onSelectLezione(props.row.original.in, e.target.checked);
              }}
            />
          );
        },
      },
      {
        ...
        Cell: (props) => {
          return (
            <div>
              {props.row.original.orarioDiInizio.toLocaleDateString(undefined, {
                year: "numeric",
                month: "2-digit",
                day: "2-digit",
              }) +
                ", " +
                props.row.original.orarioDiInizio.toLocaleTimeString(
                  undefined,
                  { hour: "2-digit", minute: "2-digit" }
                ) +
                "-" +
                props.row.original.orarioDiFine.toLocaleTimeString(undefined, {
                  hour: "2-digit",
                  minute: "2-digit",
                })}
            </div>
            ...
            >
              ...
              {props.row.original.tipoLezione === "NORMALE" && (
                <>
                  ...
                  <option value="ASSENTE_NON_GIUSTIFICATO">
