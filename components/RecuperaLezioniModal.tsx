...
type Props = {
    ...
    handleSubmit: (lezioni: LezioneDiRecupero) => Promise<{ success: boolean, errMsg?: string }>,
}

export default function RecuperaLezioneModal({ show, handleClose, lezioniDaRecuperare, handleSubmit }: Props) {
        ...
        <Modal show={show} onHide={handleClose}>
            ...
            <Form onSubmit={async (e) => {
                            ...
                            <Form.Select required name="lezioneDaRecuperare">
                                ...
                                {lezioniDaRecuperare.map(lezione => (
                                    <option key={lezione.id} value={lezione.id}>{
                                        lezione.orarioDiInizio.toLocaleDateString(undefined, { year: 'numeric', month: '2-digit', day: '2-digit' }) +
                                        ', ' +
                                        lezione.orarioDiInizio.toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' }) +
                                        '-' +
                                        lezione.orarioDiFine.toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' }) +
                                        ' ' +
                                        lezione.alunni.map(alunno => alunno.nome + ' ' + alunno.cognome).join(', ')
                                    }</option>
                                ))}
                            </Form.Select>
                        </Col>
                ...
                <Modal.Footer>
                    ...
                    <Button type="submit" disabled={executing} variant="primary">Recupera</Button>
                </Modal.Footer>
            </Form>
        </Modal>
    );
