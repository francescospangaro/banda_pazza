...
export default function GenericTable<T extends object = {}>({
  ...
  const table = (
    <Table
      className="table table-striped table-hover kill-table-header"
      {...getTableProps()}
    >
      {isNotMobile && (
        <Thead>
      ...
      <Tbody {...getTableBodyProps()}>
        {rows.map((row) => {
          ...
          const maxRows = Array.from(rows.values()).reduce((p, v) =>
            p > v ? p : v
          );
          ...
          return Array.from(Array(maxRows).keys()).map((cellRowIndex) => (
            <Tr key={key + "_" + cellRowIndex} {...restRowProps}>
              {row.cells.map((cell) => {
                ...
                const cellRows =
                  (cell.column.rowSpan ? rows.get(cell) : undefined) ?? 1;
                if (cellRows < cellRowIndex + 1) return null;
